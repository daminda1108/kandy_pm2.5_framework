...
| **"precipitation is an unmeasured gap"** | registered and tested: bottom rung **-0.129% [-4.56, +5.78]**, gains above **+0.000** | a **measured null** (F.112); no F.84 repeat |
| **the deep-tropical margin as a stable quantity** | it moves with the satellite stream (F.97), the loss (F.109) AND the driver set (F.112: +19.84 -> +9.20) | direction robust, **magnitude is not** |
